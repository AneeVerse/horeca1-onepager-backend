...
const getLogoPath = () => {
  ...
  // The logo.png is in the backend root, so we need to go up from lib/email-sender
  const possiblePaths = [
    path.join(__dirname, '../../logo.png'),  // From lib/email-sender
    path.join(process.cwd(), 'logo.png'),    // From process working directory
    path.join(__dirname, '../../../logo.png'), // Alternative path
    'logo.png'  // Fallback to relative path
  ];
